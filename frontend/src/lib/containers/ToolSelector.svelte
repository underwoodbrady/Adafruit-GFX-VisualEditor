<script lang="ts">
    ...
    let additionalInfo = ["paint-brush", "paint-bucket", "text"]
    let hasAdditionalInfo = (tool:tool) => tool.name == 'paint-brush' || tool.name == 'paint-bucket' || tool.name == 'text';

</script>

{#each tools as tool (tool.name)}
    <Tool icon={tool.image} onClick={() => {updateSelectedTool(tool)}} selected={tool == selectedTool} additionalInfo={hasAdditionalInfo(tool)} />
{/each}
